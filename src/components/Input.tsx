...
export function Input({ label, helper, error, iconLeft, iconRight, className = '', ...props }: InputProps) {
  const baseClasses = `w-full py-3 px-3.5 min-h-[48px] rounded-[var(--radius-md)] border-[1.5px] border-[var(--border-input)] bg-white text-[15px] font-[DM_Sans,sans-serif] text-[var(--text-primary)] transition-all duration-150 outline-none placeholder:text-[var(--text-muted)] focus:border-[var(--primary)] focus:shadow-[0_0_0_3px_rgba(27,152,224,0.12)] box-border`
...
export function Select({ label, options, className = '', ...props }: SelectProps) {
  return (
    <div>
      ...
      <select
        className={`w-full py-3 px-3.5 pr-[38px] min-h-[48px] rounded-[var(--radius-md)] border-[1.5px] border-[var(--border-input)] bg-white text-[15px] font-[DM_Sans,sans-serif] text-[var(--text-primary)] transition-all duration-150 outline-none appearance-none cursor-pointer focus:border-[var(--primary)] focus:shadow-[0_0_0_3px_rgba(27,152,224,0.12)] bg-no-repeat bg-[right_12px_center] bg-[length:16px] ${className}`}
        style={{
...
export function Textarea({ label, className = '', ...props }: TextareaProps) {
  return (
    <div>
      {label && <label className="block text-[13px] font-semibold text-[var(--text-primary)] mb-1.5">{label}</label>}
      <textarea
        className={`w-full py-3 px-3.5 rounded-[var(--radius-md)] border-[1.5px] border-[var(--border-input)] bg-white text-[15px] font-[DM_Sans,sans-serif] text-[var(--text-primary)] transition-all duration-150 outline-none placeholder:text-[var(--text-muted)] focus:border-[var(--primary)] focus:shadow-[0_0_0_3px_rgba(27,152,224,0.12)] resize-y box-border ${className}`}
        {...props}
      />
    </div>
  )
